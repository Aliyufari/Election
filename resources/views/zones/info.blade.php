              ...
              <div class="tab-content p-10">
                ...
                <div class="tab-pane fade show active profile-overview" id="profile-overview">
                  <h6 class="mt-3" style="color:grey;">Select election to view accreditation:</h6>
                    @forelse($elections as $data)
                      @if(strtolower($election) === strtolower($data->type))
                       <div class="row">
                          <div class="col-lg-6 col-md-6 label ">
                            <i class="bi bi-search"></i>
                            <a href="/zones/{{$zone->id}}/lgas/?name={{$data->type}} - {{$data->date}} - {{$zone->name}} - {{'Accreditation Panel'}}">{{$data->type}} - {{$data->date}} - {{$zone->name}}</a>
                          </div>
                        </div>
                      @endif
                    @empty
